...
let run_player_action_state pigeon_pos input =
  ...
  let test_world = GameWorld.put_entity origin_player_world test_pigeon in
  let test_state =
...
let rec find_pigeon_health entity_lst =
  ...
  | e :: tl ->
      if e.GameEntity.entity_type = Pigeon then e.GameEntity.stats.health
