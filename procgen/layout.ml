...
let gen_layout n =
  ...
  let layout = Array.make_matrix side_length side_length false in
  let frontier = ref [] in
  ...
  for i = 1 to n do
    ...
    frontier :=
      List.mapi (fun i x -> if i = rand_index then None else Some x) !frontier
      |> List.filter_map Fun.id;
    set_at_vec layout spot true;
    let false_neighbors =
      cardinal_neighbors spot
      |> List.filter (fun x -> get_at_vec_opt layout x = Some false)
    in
